...
export function buildConsultationSchema(t: Translate) {
  return buildContactFieldsSchema(t)
    ...
    .extend({
      trainingNeeds: z.string().trim().max(2000, t('validation.tooLong', { max: 2000 })).optional().or(z.literal('')),
      participantsCount: z
        .string()
        .trim()
        .optional()
      ...
      areasOfInterest: z.string().trim().max(500, t('validation.tooLong', { max: 500 })).optional().or(z.literal('')),
