...
import {
  buildRequestInformationSchema,
  type RequestInformationValues,
} from '@/features/leads/schemas/request-information.schema'
import { useTrainings } from '@/features/trainings/hooks/useTrainings'
import type { SupportedLanguage } from '@/i18n'

/**
 * Request Information (spec M5 §1). Training preselected from
 * `?training=<slug>` — the exact query param `TrainingDetailsPage`'s CTAs
 * already pass — resolved to `trainingId` once the training list loads
 * (slug is what's in the URL; `Lead.trainingId` is an id, matching every
 * other domain's `*Id` convention).
 */
export default function RequestInformationPage() {
  const { t, i18n } = useTranslation('requestInformation')
  ...
  const schema = useMemo(() => buildRequestInformationSchema(tFields), [tFields])
  const {
    register,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<RequestInformationValues>({
  ...
  function onSubmit(values: RequestInformationValues) {
    mutation.mutate({
      lead: {
