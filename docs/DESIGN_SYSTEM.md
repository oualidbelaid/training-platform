...
- **Logo** — `src/config/brand.ts` exports `BRAND.logo`, resolving to `src/assets/brand/istam-logo.png` (the client-supplied file, also kept at its original drop location `src/assets/images/ISTAM logo.png`). `Navbar` and `Footer` (including the mobile `Drawer` header) all render this same file at different heights — there is only one lockup (full mark + wordmark). **To update the logo:** replace `src/assets/brand/istam-logo.png` (or point `BRAND.logo` at a new import) — no component changes needed.
- **Favicon** — `public/favicon.png` (64×64) and `public/apple-touch-icon.png` (180×180), referenced from `index.html`. Since no separate icon-only source asset exists, both are a square crop of the same lockup's icon mark (the burgundy/charcoal shape to the left of the "ISTAM" wordmark) — cropped and resized programmatically from `src/assets/brand/istam-logo.png`, not hand-drawn or invented artwork. Replaces the default Vite favicon that shipped from M0 through this pass. **To update:** re-crop from a new `istam-logo.png` the same way (icon-mark bounding box, square, padded), or drop in a real client-supplied icon-only export if one is ever provided.
...
Centralized image registry: `src/config/media.ts`, mapping semantic keys to imports — components only ever consume `training.image` / `trainer.photoUrl` through the `Image` primitive; none of them import `media.ts` or know where a file physically lives.
...
`testimonial` remains the locally generated navy→now-burgundy/charcoal abstract decorative texture from the earlier redesign pass — it was never meant to be literal photography, so it was left alone. (The `events` photo/key existed here until the Events feature was removed entirely — see `docs/ROADMAP.md`.)

Every domain entity that needs a photo carries a plain `image: string` (Training, required) field, populated in mock data from `MEDIA.*` — **exactly the shape a future Dolibarr DTO will hand back** (a URL string).
...
| `Card` + `CardImage` + `CardContent` + `CardFooter` | Surface/border/radius/shadow + optional hover-depth. The primitive every business card (TrainingCard, TrainerCard, ArticleCard, etc.) composes — **do not build those business cards from scratch**.                                                                                                                                                                           |
